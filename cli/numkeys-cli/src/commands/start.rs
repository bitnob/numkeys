...
impl StartCmd {
    pub fn execute(self) -> anyhow::Result<()> {
        ...
        if !config_dir.join("config").join("issuer.toml").exists() {
            ...
            println!("\nRun 'numkeys setup' first to create an issuer configuration");
            return Ok(());
        }

        output::info(&format!("Starting NumKeys issuer node ({})", self.name));
        ...
        if self.daemon {
            ...
            if pid_file.exists() {
                ...
                output::warning(&format!(
                    "Issuer may already be running (PID: {})",
                    pid.trim()
                ));
                println!("Run 'numkeys stop' to stop the existing instance");
                return Ok(());
            }
            ...
            let child = Command::new(&issuer_bin)
                .env("NUMKEYS_CONFIG_DIR", &config_dir)
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .spawn()?;
            ...
            output::success(&format!(
                "Issuer node started in background (PID: {})",
                child.id()
            ));
            println!("\nEndpoints:");
            println!("  http://localhost:3000/attest");
            println!("  http://localhost:3000/.well-known/numkeys/pubkey.json");
            println!("\nRun 'numkeys stop' to stop the issuer");
        } else {
