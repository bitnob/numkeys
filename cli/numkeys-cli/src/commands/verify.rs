...
use crate::output;
use colored::*;
use numkeys_core::{parse_attestation_jwt, verify_attestation};
...
pub async fn execute(attestation: &str, expected_phone: Option<&str>) -> anyhow::Result<()> {
    ...
    match verify_attestation(&jwt).await {
        Ok(verified) => {
            ...
            println!(
                "  Proxy number: {}",
                verified.attestation.proxy_number.to_string().yellow()
            );
            println!(
                "  User pubkey:  {}",
                verified.attestation.user_pubkey.to_base64()
            );
            println!(
                "  Expires:      {}",
                verified
            ...
            if let Some(phone) = expected_phone {
                ...
                let expected_hash = numkeys_crypto::hash_phone_number_spec(&expected);
                let attestation_hash =
                    format!("sha256:{}", verified.attestation.phone_hash.to_hex());
