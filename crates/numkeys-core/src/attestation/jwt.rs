...
use numkeys_crypto::{sign_message, verify_signature};
use numkeys_types::{NumKeysError, NumKeysResult, PrivateKey, PublicKey, Signature};
...
pub fn encode_jwt<T: Serialize>(claims: &T, private_key: &PrivateKey) -> NumKeysResult<String> {
    // Create header
    let header = Header::default();
    let header_json = serde_json::to_string(&header)
        .map_err(|e| NumKeysError::SerializationError(e.to_string()))?;
...
pub fn decode_jwt<T: for<'de> Deserialize<'de>>(
    ...
    let header_json = general_purpose::URL_SAFE_NO_PAD
        .decode(parts[0])
        .map_err(|_| NumKeysError::InvalidAttestation("Invalid header encoding".to_string()))?;
    let header: Header = serde_json::from_slice(&header_json)
        .map_err(|e| NumKeysError::InvalidAttestation(format!("Invalid header: {}", e)))?;

    if header.alg != "EdDSA" {
        return Err(NumKeysError::InvalidAttestation(format!(
    ...
    let signature_bytes = general_purpose::URL_SAFE_NO_PAD
        .decode(parts[2])
        .map_err(|_| NumKeysError::InvalidAttestation("Invalid signature encoding".to_string()))?;
    ...
    let claims_json = general_purpose::URL_SAFE_NO_PAD
        .decode(parts[1])
        .map_err(|_| NumKeysError::InvalidAttestation("Invalid claims encoding".to_string()))?;
    serde_json::from_slice(&claims_json)
        .map_err(|e| NumKeysError::InvalidAttestation(format!("Invalid claims: {}", e)))
}

/// Decode JWT without verification (for parsing).
pub fn decode_jwt_unverified<T: for<'de> Deserialize<'de>>(jwt: &str) -> NumKeysResult<T> {
    // Split JWT
    let parts: Vec<&str> = jwt.split('.').collect();
    if parts.len() != 3 {
        return Err(NumKeysError::InvalidAttestation(
