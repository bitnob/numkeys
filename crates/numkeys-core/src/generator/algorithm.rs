//! Proxy number generation algorithm according to NumKeys Protocol specification v1.0

use crate::generator::ProxyGenerationInput;
use numkeys_crypto::sha256;
use numkeys_types::{NumKeysError, NumKeysResult, ProxyNumber};
use regex::Regex;

/// Generate proxy number following the exact specification algorithm.
pub fn generate(input: &ProxyGenerationInput) -> NumKeysResult<ProxyNumber> {
...
fn validate_inputs(input: &ProxyGenerationInput) -> NumKeysResult<()> {
    ...
    if input.issuer_domain.is_empty() {
        return Err(NumKeysError::CryptoError(
...
fn format_proxy_number(scope: &str, digits: &str) -> NumKeysResult<ProxyNumber> {
