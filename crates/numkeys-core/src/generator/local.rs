...
pub fn generate_local_proxy(namespace: &str, index: u64, country_code: &str) -> NumKeysResult<ProxyNumber> {
    // Validate country code
    if country_code.is_empty() || country_code.len() > 3 {
        return Err(numkeys_types::NumKeysError::InvalidProxyNumber(
