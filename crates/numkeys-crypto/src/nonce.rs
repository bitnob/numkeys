...
use hex;
use numkeys_types::{Nonce, NumKeysError, NumKeysResult};
...
pub fn generate_timestamped_nonce() -> NumKeysResult<Nonce> {
    let timestamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map_err(|_| NumKeysError::CryptoError("System time error".to_string()))?
        .as_secs();
...
impl NonceTracker {
    ...
    pub fn use_nonce(&self, nonce: &Nonce) -> NumKeysResult<()> {
        let mut used = self
            .used_nonces
            .lock()
            .map_err(|_| NumKeysError::CryptoError("Lock poisoned".to_string()))?;

        if used.contains(nonce.as_str()) {
            return Err(NumKeysError::InvalidNonce);
        }
...
pub fn validate_timestamped_nonce(nonce: &Nonce, max_age_seconds: u64) -> NumKeysResult<()> {
    let nonce_str = nonce.as_str();

    // Parse timestamp from nonce format: timestamp_randomdata
    let Some((timestamp_str, random_data)) = nonce_str.split_once('_') else {
        return Err(NumKeysError::InvalidNonce);
    };
    ...
    let timestamp = timestamp_str
        .parse::<u64>()
        .map_err(|_| NumKeysError::InvalidNonce)?;
...
mod tests {
    ...
    #[test]
    fn test_timestamped_nonce_allows_underscores_in_random_segment() {
