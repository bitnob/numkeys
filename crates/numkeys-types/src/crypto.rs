...
use crate::error::{NumKeysError, NumKeysResult};
...
impl PublicKey {
    ...
    pub fn try_from_slice(slice: &[u8]) -> NumKeysResult<Self> {
        if slice.len() != 32 {
            return Err(NumKeysError::InvalidPublicKey(format!(
    ...
    /// Parse from base64.
    pub fn from_base64(s: &str) -> NumKeysResult<Self> {
        let bytes = general_purpose::URL_SAFE_NO_PAD
            .decode(s)
            .map_err(|e| NumKeysError::InvalidPublicKey(e.to_string()))?;
...
impl PrivateKey {
    ...
    pub fn from_base64(s: &str) -> NumKeysResult<Self> {
        let bytes = general_purpose::URL_SAFE_NO_PAD
            .decode(s)
            .map_err(|e| NumKeysError::InvalidPrivateKey(e.to_string()))?;

        if bytes.len() != 32 {
            return Err(NumKeysError::InvalidPrivateKey(format!(
                "Expected 32 bytes, got {}",
                bytes.len()
            )));
        }
...
impl KeyPair {
    ...
    /// This requires the numkeys-crypto crate functionality.
    pub fn from_private_key(_private_key: &PrivateKey) -> NumKeysResult<Self> {
        // This will be implemented by numkeys-crypto
        Err(NumKeysError::CryptoError(
            "KeyPair::from_private_key must be called through numkeys-crypto".to_string(),
        ))
    }
}
...
impl Signature {
    ...
    pub fn try_from_slice(slice: &[u8]) -> NumKeysResult<Self> {
        if slice.len() != 64 {
            return Err(NumKeysError::InvalidSignature);
        }
    ...
    pub fn from_base64(s: &str) -> NumKeysResult<Self> {
        let bytes = general_purpose::URL_SAFE_NO_PAD
            .decode(s)
            .map_err(|_| NumKeysError::InvalidSignature)?;
        Self::try_from_slice(&bytes)
    }
}
...
/// Ed25519 binding proof.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct BindingProof(#[serde(with = "base64_serde")] [u8; 64]);
...
impl BindingProof {
    ...
    /// Parse from base64url without padding.
    pub fn from_base64(s: &str) -> NumKeysResult<Self> {
        let bytes = general_purpose::URL_SAFE_NO_PAD
            .decode(s)
            .map_err(|_| NumKeysError::InvalidBindingProof)?;

        if bytes.len() != 64 {
            return Err(NumKeysError::InvalidBindingProof);
        }

        let mut array = [0u8; 64];
        array.copy_from_slice(&bytes);
        Ok(BindingProof(array))
    }
}
