# NumKeys Issuer Node Specification

**Version**: 1.0  
**Status**: Definitive  
**Purpose**: Protocol requirements for entities that issue NumKeys attestations
...
```
cc = scope  // Calling code (1-4 digits like "1", "44", or "1264")
...
### 3.4 Binding Proof (v1.1 - Signature Based)

The binding_proof MUST be computed as:
```
binding_proof = "sig:" + base64url(Ed25519-Sign(issuer_private_key, UTF8(message)))

where message = "numkeys-binding" + "|" + iss + "|" + sub + "|" + phone_hash + "|" + user_pubkey + "|" + nonce + "|" + iat + "|" + jti
```
...
- `sub`: The JWT subject claim containing the full proxy number with '+' prefix (e.g., "+10012345678")
- `nonce`: The exact JWT `nonce` value used for proxy generation
- `iat`: Unix timestamp as decimal string (e.g., "1720000000")
- `jti`: The exact JWT ID string
- `"numkeys-binding"`: Fixed domain-separation prefix
...
Message: "numkeys-binding|issuer.example.com|+10012345678|sha256:c775e7b757ede630cd0aa1113bd102661ab38829ca52a6422ab782862f268646|MCowBQYDK2VwAyEAa7bsa2eI7T6w9P6KVJdLvmSGq2uPmTqz2R0RBAl6R2E=|0123456789abcdef0123456789abcdef|1720000000|11111111-1111-4111-8111-111111111111"

Ed25519-Sign(key, UTF8(message)): [64 bytes]
...
3. All JWT claims and issued-at policy checks
...
```
https://{issuer_domain}/.well-known/numkeys/pubkey.json
```
...
2. Returns a signed attestation after the issuer's phone-verification process succeeds
...
- Total length between 8-15 characters
- Valid regex: `^\+[1-9]\d{6,14}$`

**Scope Requirements**:
- Must be provided (required parameter)
- Must be 1-4 digit calling code (e.g., "1", "44", or "1264")
- Must be a supported calling code for local proxy generation
