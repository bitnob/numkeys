# NumKeys Protocol Specification
...
NumKeys is a protocol that enables users to share proxy phone numbers instead of real phone numbers. These proxy numbers are cryptographically verifiable, proving they represent a real phone number without revealing it.
...
`exp` MAY be present for issuer-specific metadata, but core verification freshness is driven by `iat` and verifier policy.
...
1. **Service detects proxy** → Recognizes the `+{country_code}00...` pattern
...
4. **Wallet signs canonical response payload**:
   ```json
   {
     "service_id": "app.example.com",
     "challenge_nonce": "a1b2c3d4e5f6",
     "response_nonce": "f6e5d4c3b2a1",
     "verification_id": "verify_12345",
...
2. Fetch public key from `https://{issuer}/.well-known/numkeys/pubkey.json`
...
2. Reconstruct canonical JSON bytes of the response payload
3. Verify the user's signature over those bytes
4. Check `service_id`, `challenge_nonce`, and `verification_id` match the original challenge
5. Confirms: User personally approved this verification
...
- **Discovery**: Public key at `/.well-known/numkeys/pubkey.json`
...
```
GET https://issuer.example.com/.well-known/numkeys/pubkey.json
...
- **Structured payloads**: Signatures cover canonical JSON containing service, challenge, and verification identifiers
- **Timestamps**: Responses include current timestamp in Unix milliseconds
...
- **Creation**: 
  - Message: `numkeys-binding + "|" + iss + "|" + sub + "|" + phone_hash + "|" + user_pubkey + "|" + nonce + "|" + iat + "|" + jti`
  - Signature: `Ed25519-Sign(issuer_private_key, UTF8(message))`
...
4. **Sign response payloads** → Cryptographic proofs
...
- **HTTPS Required**: All production endpoints MUST use HTTPS
  - Key discovery: `/.well-known/numkeys/pubkey.json`
...
The NumKeys Protocol provides a complete system for privacy-preserving phone number verification. By separating identity (real number) from identifier (proxy number) and using cryptographic proofs, users can share contact information without compromising privacy.

This specification defines all necessary components for implementing NumKeys-compliant issuers, wallets, and services. The protocol is designed to be simple to understand, secure by default, and respectful of user privacy.
