...
async fn main() -> anyhow::Result<()> {
    ...
    let issuer_key = if let Some(key_path) = &config.private_key_path {
        ...
        numkeys_crypto::keypair_from_private(&private_key)?
    } else {
        // Generate new
        generate_keypair()?
    };
    tracing::info!("Issuer public key: {}", issuer_key.public.to_base64());
    ...
    let app = Router::new()
        ...
        .route("/.well-known/numkeys/pubkey.json", get(api::pubkey::pubkey))
    ...
    tracing::info!("  GET    /.well-known/numkeys/pubkey.json - Public key discovery");
...
mod tests {
    ...
    async fn test_attestation_issuance() {
        ...
        // Create test client
        let client = tower::ServiceBuilder::new().service(app);
